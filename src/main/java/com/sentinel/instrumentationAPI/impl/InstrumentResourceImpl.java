    ...
    public GetInstrumentResponse getInstrument() throws Exception {
        ...
        StreamingOutput streamingOutput = new StreamingOutput() {
            ...
            public void write(OutputStream output) throws IOException, WebApplicationException {
                ...
                List<InstrumentedApk> instrumentedApkList = new ArrayList<InstrumentedApk>();
                while(apkIterator.hasNext()) {
                    String hash = apkIterator.next();
                    instrumentedApkList.add(new InstrumentedApk().withHash(hash).withUrl("http://sentinel.de/instrumentedApps/" + hash));
                }
                Apks apks = new Apks().withApks(instrumentedApkList);

                for(InstrumentedApk apk : apks.getApks()) {
                    System.out.println(apk.toString());
                    writer.write(apk.toString()+ "\n");
                }
